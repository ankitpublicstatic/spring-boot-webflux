...
public class ProductService {
  ...
  public Mono<ProductDto> getProduct(String id) {
    return productRepository.findById(id).map(AppUtils::entityToDto).switchIfEmpty(
        Mono.error(new ProductNotFoundException("Product not found with given id : " + id)));
  }
  ...
  public Mono<ProductDto> saveProduct(Mono<ProductDto> productDto) {
    return productDto.map(AppUtils::dtoToEntity).flatMap(productRepository::insert)
